...
# Train random forest with B trees of 2~p features
rf_cv <- train(
  form_rf, 
  data = train_ori,
  method = "rf",
...
```{r}
dtrain <- xgb.DMatrix(data = as.matrix(train_raw %>% select(-charges)), label = train_raw$charges)
params <- list(
  booster = "gbtree",
...
```{r}
dtest <- xgb.DMatrix(data = as.matrix(test_raw %>% select(-charges)), label = test_raw$charges)
pred <- predict(final_model, dtest)

# 4. 驗證準確度 (用原本的金額來算 RMSE)
rmse <- sqrt(mean((test_raw$charges - pred)^2))

cat("--- 模型評估 ---\n")
cat("測試集 RMSE (美金):", rmse, "\n")

# 算一下 R-squared 看看有沒有進步
rss <- sum((test_raw$charges - pred) ^ 2)
tss <- sum((test_raw$charges - mean(test_raw$charges)) ^ 2)
r_sq <- 1 - rss/tss
cat("R-squared (解釋力):", r_sq, "\n")
```

```{r}
results <- data.frame(preds = pred, actual = test_raw$charges)
results$residuals <- results$actual - results$preds
