...
Insurance* dataset from Kaggle, consisting of **2,706 observations and 7
...
utilize stepwise selection algorithms to identify the optimal subset of
predictors.
...
```{r}
set.seed(123)
regfit.fwd = regsubsets(charges~.,data = train_set,method = 'forward', nvmax = 12)
reg.fwd.summary = summary(regfit.fwd)
reg.fwd.summary
fwd_num_models <- length(reg.fwd.summary$rss)
```
...
points(index,reg.fwd.summary$adjr2[index],col = "red", pch=19)
...
points(index,reg.fwd.summary$bic[index],col = "red", pch=19)
```
...
test.mat = model.matrix(charges~.,data = test_set)
test.errors = rep(NA,fwd_num_models)
for(i in 1:fwd_num_models){
...
points(min.index, sqrt(reg.fwd.summary$rss[min.index]/n),col = "red", pch=19)

min.index = which.min(test.errors)
cat("\nSize:", min.index, "\n")
cat("Test RMSE:", test.errors[min.index],"\n")
coef(regfit.fwd,id=min.index)
plot(1:fwd_num_models, test.errors,xlab='Number of variables', ylab='Test RMSE',type="b", pch=19)
points(min.index, test.errors[min.index],col = "red", pch=19)
```
...
```{r}
set.seed(123)
regfit.bwd = regsubsets(charges~.,data = train_set,method = 'backward', nvmax = 12)
reg.bwd.summary = summary(regfit.bwd)
reg.bwd.summary
bwd_num_models <- length(reg.bwd.summary$rss)
```
...
points(index,reg.bwd.summary$adjr2[index],col = "red", pch=19)
...
points(index,reg.bwd.summary$bic[index],col = "red", pch=19)
```

```{r}
par(mfrow = c(1, 2))
test.mat = model.matrix(charges~.,data = test_set)
test.errors = rep(NA,bwd_num_models)
for(i in 1:bwd_num_models){
...
points(min.index, sqrt(reg.bwd.summary$rss[min.index]/n),col = "red", pch=19)

min.index = which.min(test.errors)
cat("\nSize:", min.index, "\n")
cat("Test RMSE:", test.errors[min.index],"\n")
coef(regfit.bwd,id=min.index)
plot(1:bwd_num_models, test.errors,xlab='Number of variables', ylab='Test RMSE',type="b", pch=19)
points(min.index, test.errors[min.index],col = "red", pch=19)
```
...
```{r}
predictors_fs <- setdiff(names(train_set), c('charges', 'regionnorthwest'))
form_fs <- as.formula(
...
R2_train_fs <- computeR2(train_pred_fs, y_train, TRUE)
#  1 - sum((y_train - train_pred)^2) / sum((y_train - mean(y_train))^2)
R2_test_fs  <- computeR2(test_pred_fs, y_test, TRUE) 
#  1 - sum((y_test - test_pred_fs)^2) / sum((y_test - mean(y_test))^2)

# Compute RMSE
RMSE_train_fs <- computeRMSE(train_pred_fs, y_train, TRUE)
RMSE_test_fs  <- computeRMSE(test_pred_fs, y_test, TRUE)

cat('Train R^2:',R2_train_fs,'\n')
cat('Test R^2:',R2_test_fs,'\n')
cat('Train RMSE:',RMSE_train_fs,'\n')
cat('Test RMSE:',RMSE_test_fs,'\n')
```

##### Model Conclusion

Overall Model Performance The linear regression model fits the data
...
Impact of Log-Transformation Since the dependent variable is transformed
...
-   Smoking Status: This is the most dominant factor. The base
    coefficient for smokeryes (1.196) suggests that, holding other
    variables constant, smoking leads to a massive increase in predicted
    charges (over 200% on the original scale).

-   BMI and Smoking Interaction: While bmi alone is not statistically
    significant ($p=0.823$), the interaction term bmi:smoker is highly
...
-   Other Demographics:
    ...
    -   Children: Each additional child is associated with an increase
        in costs ($\beta \approx 0.106$).

    -   Sex: Males have slightly lower predicted costs than females
        ($\beta \approx -0.083$), holding all else constant.

    -   Region: Residents in the Southeast and Southwest have
        significantly lower costs compared to the reference region
...
model_results <- data.frame(
  Model = c("OLS baseline","Lasso (CV λ_min)","Lasso (BIC)","Feature Selection", "Random Forest", "XGBoost"), 
  R2_test = c(R2_test_lm, R2_test_cv_base, R2_test_bic_base, R2_test_fs, rf_test_R2, r_sq),
  RMSE_test = c(RMSE_test_lm, RMSE_test_cv_base, RMSE_test_bic_base, RMSE_test_fs, rf_test_rmse, rmse)
)
